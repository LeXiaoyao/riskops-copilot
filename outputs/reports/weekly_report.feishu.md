...
| --- | --- |
| 异常总数 | 8 |
| high / medium / low | 6 / 2 / 0 |
| 核心指标 | D7 回收率 |
| Top drivers | 5 |

> Demo Disclaimer: 本报告基于 synthetic_data / 合成数据生成，仅用于 Demo 展示，不代表真实业务结论。

## 1. 执行摘要

- **本期异常发现**：共发现 8 个异常，其中 high 6 个、medium 2 个、low 0 个。
- **核心问题**：M1 D7 回收率下降。M1 回收率 从 19.40% 降至 15.30%，relative change 为 -21.30%。
...
- **PTP 履约率（ptp_keep_rate）**
  ...
  - baseline / recent：50.00% / 44.00%
  - relative change：-12.00%
  - 业务解释：PTP 履约下降说明承诺还款后的跟进、客户还款意愿或策略工具支持出现压力。
  - 建议动作：检查 PTP 履约下降原因。
- **减免使用率（reduction_usage_rate）**
  - severity：high
  - dimension：overall=ALL
  - baseline / recent：0.06% / 0.04%
  - relative change：-27.00%
...
- **baseline / recent**：23.90% / 6.30%
- **contribution_score**：15.30%
- **confidence**：medium
...
- **baseline / recent**：29.50% / 12.70%
- **contribution_score**：6.50%
- **confidence**：medium
...
- **baseline / recent**：17.80% / 5.90%
- **contribution_score**：5.80%
- **confidence**：medium
...
- **M3 原始解释**：D 客群组内回收表现恶化。

### 4. province=上海

- **baseline / recent**：22.40% / 7.20%
- **contribution_score**：4.40%
- **confidence**：medium
...
- **baseline / recent**：36.40% / 19.90%
- **contribution_score**：4.00%
- **confidence**：medium
...
- **M3 原始解释**：A 客群组内回收表现恶化。

## 5. 产能压力分析

- **line_id 口径说明**：line_id 在本项目中表示催收作业线 / 催收单元 / 分案作业队列，不是电话线路，也不是单纯地理线路。
- **产能压力信号**：华东线路人均案量（avg_case_per_collector）从 13.54 升至 19.10，relative change 为 41.10%。
- **业务解释**：人均案量上升意味着催员负荷变重，是 capacity pressure signal，可能影响触达节奏和承诺还款跟进，但不是最终根因。
- **建议动作**：下钻华东各 line_id。
...
> 本文档由 RiskOps Copilot 自动生成，可直接粘贴到飞书文档 / 群消息。
