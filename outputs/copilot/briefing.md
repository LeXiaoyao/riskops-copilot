...
- **window**：baseline=2026-03-19~2026-04-17, recent=2026-04-18~2026-05-17
- **target_metric**：M1 回收率（m1_recovery_rate）
- **target_relative_change**：-21.27%

## Why It May Be Happening

- Top drivers are directional attribution signals, not final root cause.
- **channel_code=ECOM**：contribution=15.26%; action=继续按该维度下钻到供应商、线路和客群组合，验证是否存在集中异常。
- **province=山东**：contribution=6.52%; action=继续按该维度下钻到供应商、线路和客群组合，验证是否存在集中异常。
- **score_band=D**：contribution=5.75%; action=对该客群单独设定触达、减免和 PTP 跟进策略，不改变指标口径。
- **province=上海**：contribution=4.42%; action=继续按该维度下钻到供应商、线路和客群组合，验证是否存在集中异常。
- **score_band=A**：contribution=4.04%; action=对该客群单独设定触达、减免和 PTP 跟进策略，不改变指标口径。
...
- Start drilldown from `channel_code=ECOM` because it has the highest contribution score.
...
_Generated at 2026-05-25T04:59:56+00:00_
